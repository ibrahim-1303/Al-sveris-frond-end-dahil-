...
using MvC5.Migrations.SeedData;
...
namespace MvC5.Tests
{
    public abstract class MvC5TestBase : AbpIntegratedTestBase<MvC5TestModule>
    {
        ...
        protected void UsingDbContext(Action<MvC5DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MvC5DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MvC5DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MvC5DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MvC5DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MvC5DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MvC5DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MvC5DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MvC5DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MvC5DbContext, Task<T>> func)
        {
            T result;
